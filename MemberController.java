...
public class MemberController extends HttpServlet {
    ...
    protected void doGet(HttpServletRequest request, HttpServletResponse response) throws ServletException, IOException {
        ...
        HttpSession session = request.getSession(); // 있는 경우 반환, 없는 경우 만들어서 반환 서블릿에서는 세션이 default 객체가 아니기 때문에 세션
        // 개체를 가져와야 한다.
        String uri = request.getRequestURI(); // URI를 가져온다.
        String action = uri.substring(uri.lastIndexOf("/") + 1); // 문자열 짜르기
        ...
        if(action.equals("signin.do")) {
            ...
            if((retMember = dao.read(member)) != null && pw.equals(retMember.getPw())) { // db에 있는 값과 일치한다면
                //로그인 성공
